...
_CSV_HEADERS = ['algo', 'model', 'payoff', 'drift', 'volatility', 'mean',
                ...
                'ridge_coeff', 'activation', 'dropout', 'num_layers',
                'use_payoff_as_input', 'use_barrier_as_input',
                'train_ITM_only',
...
def _run_algos():
    ...
    for config_name, config in configs_getter.get_configs():
        ...
        combinations = list(itertools.product(
            ...
            config.ridge_coeff, config.activation, config.dropout, config.num_layers,
            config.train_ITM_only, config.use_payoff_as_input,
...
def _run_algo(
        ...
        factors=(1., 1., 1.), ridge_coeff=1e-3, activation='leakyrelu', dropout=0.0, num_layers=1,
    ...
    try:
        if algo in ["RLSM", "RFQI"]:
            ...
            pricer = _ALGOS[algo](
                ...
                dropout=dropout,
                num_layers=num_layers if algo == "RFQI" else 1,
            ...
            pricer = _ALGOS[algo](
                ...
                activation=activation,
                dropout=dropout,
                num_layers=num_layers if algo == "SRFQI" else 1,
                train_ITM_only=train_ITM_only,
                use_payoff_as_input=use_payoff_as_input,
                use_barrier_as_input=use_barrier_as_input,
